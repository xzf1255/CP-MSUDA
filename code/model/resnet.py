...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    # if pretrained:
    #     state_dict = torch.load('/SSD/xzf/msda/prototype/pretrained_model/resnet18-5c106cde.pth')
...
#   if pretrained == True:
#     # model_path = '/SSD/xzf/msda/prototype/pretrained_model'
#     # model_zoo.load_url(model_urls['resnet50'],model_dir=model_path)
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.load('/SSD/xzf/msda/prototype/pretrained_model/resnet50-19c8e357.pth')
        model.load_state_dict({k: v for k, v in state_dict.items() if k in model.state_dict()})
    return model
def resnet50_(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        state_dict = torch.load('/SSD/xzf/msda/prototype/pretrained_model/resnet50-19c8e357.pth')
        model.load_state_dict({k: v for k, v in state_dict.items() if k in model.state_dict()})
    return model
...
def resnet101(pretrained=False, progress=True, **kwargs):
  ...
  if pretrained == True:
    model_path = '/SSD/xzf/msda/prototype/pretrained_model/resnet101-5d3b4d8f.pth'
    print("Loading pretrained weights from %s" %(model_path))
...
class Predictor(nn.Module):
  def __init__(self, feat=2048, classes=10):
