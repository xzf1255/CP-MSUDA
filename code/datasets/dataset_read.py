...
sys.path.append('/SSD/xzf/msda/code/LtC-MSDA/datasets')
...
def dataset_read1(target,i,batch_size):
    S1 = {}
    S1_test = {}
    S2 = {}
    S2_test = {}
    ...
    if i == 'T':
        ...
        T['imgs'] = target_train
        T['labels'] = target_train_label
        dataset_size=target_train.shape[0]
        temp = T
    else:
    # for i in range(len(domain_all)):
        source_train, source_train_label, source_test , source_test_label = return_dataset(domain_all[i])
        ...
        # S_test[i]['imgs'] = source_test
        # S_test[i]['labels'] = source_test_label
        dataset_size=source_train.shape[0]
        temp = S[i]
    ...
    train_loader.initialize1(temp, batch_size, scale=scale)
    dataset = train_loader.load_data()
