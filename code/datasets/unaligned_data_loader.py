...
class PairedData1(object):
    def __init__(self, data_loader_A, max_dataset_size):
        self.data_loader_A = data_loader_A   
        self.stop_A = False
        self.max_dataset_size = max_dataset_size
    ...
    def __next__(self):
        ...
        if (self.stop_A ) or self.iter > self.max_dataset_size:
            self.stop_A = False
            raise StopIteration()
        else:
            self.iter += 1
            return {'domain': A, 'label': A_paths,
                    }
...
class UnalignedDataLoader():
    ...
    def initialize1(self, source, dataset_size, scale=32):
        transform = transforms.Compose([
            transforms.Resize(scale),
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        ...
        #dataset_source1 = Dataset(source[1]['imgs'], source['labels'], transform=transform)
        dataset_source1 = Dataset(source['imgs'], source['labels'], transform=transform)
        data_loader_s1 = torch.utils.data.DataLoader(dataset_source1, batch_size=dataset_size, shuffle=True, num_workers=1)
        self.dataset_s1 = dataset_source1

        # dataset_source2 = Dataset(source[1]['imgs'], source[1]['labels'], transform=transform)
        # data_loader_s2 = torch.utils.data.DataLoader(dataset_source2, batch_size=dataset_size[2], shuffle=True, num_workers=1)
        # self.dataset_s2 = dataset_source2

        # dataset_source3 = Dataset(source[2]['imgs'], source[2]['labels'], transform=transform)
        # data_loader_s3 = torch.utils.data.DataLoader(dataset_source3, batch_size=dataset_size[3], shuffle=True, num_workers=1)
        # self.dataset_s3 = dataset_source3      

        # dataset_source4 = Dataset(source[3]['imgs'], source[3]['labels'], transform=transform)
        # data_loader_s4 = torch.utils.data.DataLoader(dataset_source4, batch_size=dataset_size[4], shuffle=True, num_workers=1)
        # self.dataset_s4 = dataset_source4     

        #for i in range(len(source)):
        #    dataset_source[i] = Dataset(source[i]['imgs'], source[i]['labels'], transform=transform)
        # dataset_target = Dataset(target['imgs'], target['labels'], transform=transform)
        # data_loader_t = torch.utils.data.DataLoader(dataset_target, batch_size=dataset_size[0], shuffle=True, num_workers=1)
        

        # self.dataset_t = dataset_target
        self.paired_data = PairedData1(data_loader_s1,
                                      float("inf"))
    def name(self):
